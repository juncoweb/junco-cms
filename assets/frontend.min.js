...
fn();window.addEventListener('scroll',fn);};const JsTheme=function($btn){const storageName='prefers-color-scheme';const currentTheme=localStorage.getItem(storageName)||'auto';function getTheme(mode){if(mode==='auto'){return window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';}
return mode;}
...
$el.addEventListener('click',function(){localStorage.setItem(storageName,mode);document.body.click();set(mode);});});};const Navbar=function(nav,btn,options){if(typeof nav=='string'){nav=document.querySelector(nav);}
...
return{minimizer:function(slc){minimizer=document.querySelector(slc);minimizer.addEventListener('click',function(){setCookie(!body.classList.contains('navbar-minimized'));minimizerFn();});minimizerFn();}}};const Frontend=(function(){const controls=JsControls({tpl:{}});return{attach:function(name,fn){controls.attach('tpl',name,fn);},attachAll:function(obj){controls.attachAll('tpl',obj);},load:function(box){controls.load('tpl',box);}};})();Frontend.attachAll({logout:function(el){el.addEventListener('click',function(event){UsysLogout();});},theme:JsTheme,language:function($btn){$btn.parentNode.querySelectorAll('[data-value]').forEach(function(el){const lang=el.getAttribute('data-value');el.addEventListener('click',function(){JsRequest.xjs({url:JsUrl('language/change'),data:{lang},onSuccess:function(res){if(res.ok()){window.location.reload();}else{alert(res.message);}},});});});},notifications:function(el){JsNotifications.load(el);el.addEventListener('click',function(){JsNotifications.show();});},search:(function(){let box;return function(el){el.addEventListener('click',function(event){event.preventDefault();if(!box){box=document.body.appendChild(JsElement('DIV.tpl-search',{html:'<div><i class="fa-solid fa-xmark cursor-pointer"></i></div>'
...
const data=get(key);if(data){show(data.total);}else{request();}}};})();window.addEventListener('load',function(){const el=document.querySelector('#cookieconsent');if(el){if(!JsCookie.get('cookieConsent')){el.classList.add('visible');document.querySelector('#cc-btn').addEventListener('click',function(){el.classList.remove('visible');JsCookie.set('cookieConsent',1);});}}});
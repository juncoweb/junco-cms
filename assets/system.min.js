...
const promise=(extension==='js')?getElementPromise('script',{async:true,src:resource},resource):getElementPromise('link',{type:'text/css',rel:'stylesheet',href:resource},resource);All.push(promise);promises[resource]={complete:false,promise:promise};}});return Promise.all(All);};})();function Lightbox(force,options){let overlay=document.body.appendChild(JsElement('div.lightbox',{html:'<div></div><i class="fa-solid fa-xmark lightbox-cross"></i>'}));let el=overlay.firstChild;options=Object.assign({hideOnlyWithButton:false,valignCenter:true,},options);function cross(btn,box){btn.addEventListener('click',function(){el.hide();});box.addEventListener('click',function(event){if(event.target!==box){event.stopPropagation();}});}
if(options.valignCenter){overlay.classList.add('valign-center');}
if(options.hideOnlyWithButton){cross(overlay.querySelector('.lightbox-cross'),overlay);}else{cross(overlay,el);}
el.destroy=function(){overlay.parentNode.removeChild(overlay);document.body.classList.remove('lightbox-fixed');};el.toggle=function(force){overlay.style.display=document.body.classList.toggle('lightbox-fixed',force)?'':'none';};el.show=function(){this.toggle(true);};el.hide=function(){this.toggle(false);};el.toggle(force);return el;};function JsLoading(force){let ID='js-loading';let doc=window.self.document;(doc.getElementById(ID)||doc.body.appendChild(JsElement('div',{id:ID,className:ID}))).style.display=force?'':'none';};var JsMove=function(el,startFn,moveFn,endFn){function setClient(event){var e=event;switch(event.type){case'touchstart':e=event.touches[0];break;case'touchmove':case'touchend':e=event.changedTouches[0];break;}
...
tooltip.style.top=top-rectC.top+'px';tooltip.style.left=left-rectC.left+'px';}
...
events(el,[0,1,1,0]);if(delay){events(tooltip,[0,2]);}};Array.from(box.querySelectorAll('[data-tooltip]')).forEach(tt);};const JsFelem=(function(){const _observe={};const _felem={submit:function(el){el.addEventListener(el.getAttribute('data-value')||'click',function(){JsFelem.submit(el);});}};const that={implement:function(handles){for(let handle in handles){if(typeof handles[handle]=='function'){_felem[handle]=handles[handle];}}},observe:function(handles){for(let handle in handles){if(typeof handles[handle]=='function'){_observe[handle]=handles[handle];}}},load:function(box,data){if(!box){box=document;}else if(typeof box==='string'){box=document.querySelector(box);}
...
this.log(this.getTextData(html));},};})();var JsUrl=(function(){var options=JsOptions('url').get({useRewrite:false,setLang:false,lang:'lang',go:'goto',format:'format',});function toQueryString(args){let arr=[];for(let name in args){arr.push(name+'='+encodeURIComponent(args[name]));}
return arr.join('&');}
...
JsRequest.json({url:options.url,data:JsRequest.mergeData(options.data,{q:q}),onSuccess:function(json){_callback(q,json);},});};})();var cropImage=function(src,callback,options,LastPosition){let image=new Image();image.src=src;image.onload=function(){let isProportional;let limits={};let minWH=40;let lightbox=Lightbox(true,{hideOnlyWithButton:true});let wrapper=lightbox.appendChild(JsElement('div.crop-image',{html:'<div><div><div></div><div></div><div></div><div></div></div></div><button class="btn btn-primary btn-solid btn-block">OK</button>'}));let container=wrapper.querySelector('div');let frame=container.querySelector('div');let handle=frame.querySelectorAll('div');let enter=wrapper.querySelector('button');function set(values){for(let i in values){limits[i]=values[i];}}
...
let v=0;let H=document.querySelector('header[data-sticky]');function fn(){let top=H?H.getBoundingClientRect().height:0;if(v!=(document.documentElement.scrollTop>top)){v=el.classList.toggle('active');el.style.top=top+'px';}}

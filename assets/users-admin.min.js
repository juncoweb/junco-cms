...
return{List:function(){function callback(message,code){if(code){_backlist.refresh();}
_backlist.notify(message);};let _backlist=Backlist().url($U).controls({status:{onSuccess:callback,},}).allowHistory().load();},};})();let UsersActivities=(function(){function $U(task){return JsUrl('admin/users.activities/'+task);}
return{List:function(){function callback(message,code){if(code){if(target){target=target.close();}
_backlist.refresh();}
(target||_backlist).notify(message);}

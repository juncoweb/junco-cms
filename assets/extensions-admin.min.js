...
function compile(_target){let f=JsForm({btn:_target});if(f){if(!f.getForm().compress){f.request({url:$U('confirm_compile'),modalOptions:{onLoad:function(){target.close();target=compile(this);},},},false,'modal');}else{f.request($U('compile'),callback);}}
return _target;}
...
function List(target){function callback(message,code){if(code){if(target_2){target_2=target_2.close();}
_backlist.refresh();}
...
let _backlist,target;let _controls={confirm_download:{modalOptions:{onLoad:function(){target=this;JsForm({btn:this}).request($U('download'),function(message,code){if(code){if(target){target=target.close();}
_backlist.refresh();}
(target||_backlist).notify(message);});}}},};return{List:function(){if(!_backlist){_backlist=Backlist().url($U).controls(_controls).allowHistory().load({onSuccess:function(){JsRating();}});}

...
el.addEventListener('click',function(){localStorage.setItem(storageName,theme);document.body.click();set(theme,className,title);});});};var Backend=(function(){const controls=JsControls({tpl:{}});return{attach:function(name,fn){controls.attach('tpl',name,fn);},attachAll:function(obj){controls.attachAll('tpl',obj);},load:function(box){controls.load('tpl',box);}};})();window.addEventListener('DOMContentLoaded',function(){const el=Navbar('.navbar','.pull-btn');if(el){el.minimizer('.navbar-minimizer > a');}
const h=document.body.querySelector('.layout-header');if(h){JsFelem.load(h);}
Backend.load(document);});Backend.attach('search',function(el){var dd=el.parentNode.parentNode.appendChild(JsElement('div'));var status=0;var data=null;var QR={'a':'[aáàâä]','e':'[eéèêë]','i':'[iíìîï]','o':'[oóòôö]','u':'[uúùûü]','n':'[nñ]'};var types=['fa-regular fa-window-maximize','fa-solid fa-gear'];function getRegExp(re){for(var i in QR){re=re.replace(RegExp(i,'gi'),QR[i]);}
return RegExp(re,'i');}
function _print(value){var html='';if(value){var regex=getRegExp(value);data.forEach(row=>{if(row[1].search(regex)!=-1){html+='<li><a href="'+row[2]+'"><i class="'+types[row[0]]+' color-light"></i>'+row[1]+'</a></li>';}});}
if(html){html='<div class="dropdown-menu"><ul>'+html+'</ul></div>';dd.innerHTML=html;}else{dd.innerHTML='';}}
el.addEventListener('input',function(){if(status===0){status=1;JsRequest.json({url:JsUrl('admin/backend/menus'),onSuccess:function(json){data=json;_print(el.value);status=2;}});}else if(status==2){_print(el.value);}});});Backend.attach('thirdbar',function(bar){(document.querySelectorAll('.widget-thirdbar ul > li > a')||[]).forEach(function($btn){const $menu=$btn.nextSibling&&$btn.nextSibling.tagName==='UL'?$btn.nextSibling:null;if($menu){$btn.setAttribute('aria-expanded',false);$btn.addEventListener('click',function(event){event.preventDefault();event.stopPropagation();JsDropdown($menu,{onToggle:function(status){$menu.classList.toggle('active',status);$btn.setAttribute('aria-expanded',status);},}).toggle();});}});});Backend.attachAll({notifications:function(el){el.addEventListener('click',function(){Notifications();});},logout:function(el){el.addEventListener('click',function(){UsysLogout();});},theme:JsTheme,});function UsysLogout(){JsDropdown.hide();JsRequest.modal({url:JsUrl('/usys/logout'),modalOptions:{onLoad:function(){JsForm('logout',{btn:this}).request(JsUrl('/usys/take_logout'),function(){window.location.reload();});}}});}

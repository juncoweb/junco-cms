...
var cropImage = function (src, callback, options, LastPosition) {
	...
	image.onload = function () {
		...
		let minWH = 40;
		//
		let lightbox = Lightbox(true, { hideOnlyWithButton: true });
		let wrapper = lightbox.appendChild(JsElement('div.crop-image', {
			html: '<div><div><div></div><div></div><div></div><div></div></div></div><button class="btn btn-primary btn-solid btn-block">OK</button>'
		}));
		let container = wrapper.querySelector('div');
		let frame = container.querySelector('div');
		let handle = frame.querySelectorAll('div');
		let enter = wrapper.querySelector('button');

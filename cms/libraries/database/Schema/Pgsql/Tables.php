...
namespace Junco\Database\Schema\Pgsql;
...
class Tables implements TablesInterface
{
    ...
    public function showData(
        ...
        // query
        $data['PgsqlQuery'] = $this->db->query("SHOW CREATE TABLE `$_tbl_name`")->fetchColumn(1);

        if ($add_if_not_exists) {
            $data['PgsqlQuery'] = preg_replace('/^CREATE TABLE/', 'CREATE TABLE IF NOT EXISTS', $data['PgsqlQuery']);
        }
        if (!$add_auto_increment) {
            $data['PgsqlQuery'] = preg_replace('/AUTO_INCREMENT=\d+ /', '', $data['PgsqlQuery']);
        }
        if ($set_db_prefix) {
            $data['PgsqlQuery'] = $this->prefixer->replaceWithUniversal($data['PgsqlQuery'], $tbl_name);
    ...
    public function create(string $TableName, array $Table): int
    {
        ...
        if ($Table['PgsqlQuery']) {
            ...
            return $this->db->exec($Table['PgsqlQuery']);
        }

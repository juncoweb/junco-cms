...
class Indexes implements IndexesInterface
{
    ...
    public function create(string $TableName, string $IndexName, array $Index): int
    {
        ...
        $has = $this->db->safeFind("SHOW INDEX FROM `$TableName` WHERE Key_name = '$IndexName'")->fetchColumn();

        if ($IndexName == 'PRIMARY') {
            if ($has) {
                return $this->db->safeExec("ALTER TABLE `$TableName` DROP PRIMARY KEY, ADD PRIMARY KEY ($Columns)");
            }
            return $this->db->safeExec("ALTER TABLE `$TableName` ADD PRIMARY KEY ($Columns)");
        }
        if ($has) {
            return $this->db->safeExec("ALTER TABLE `$TableName` DROP INDEX `$IndexName`, ADD $Index[Type] `$IndexName` ($Columns)");
        }
        return $this->db->safeExec("ALTER TABLE `$TableName` ADD $Index[Type] `$IndexName` ($Columns)");
    }
    ...
    public function drop(string $TableName, string $IndexName, array $Index = []): int
    {
        ...
        return $this->db->safeExec("ALTER TABLE `$TableName` DROP INDEX `$IndexName`");
    }
}

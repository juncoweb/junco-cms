...
use Junco\Database\Base\SanitizeTrait;
...
class Adapter implements AdapterInterface
{
    ...
    public function __construct(array $config)
    {
        try {
            ...
            switch ($config['database.adapter']) {
                case 'pdo.sqlite':
                    $this->driver = 'sqlite';
                    $dsn = 'sqlite:' . $this->getSQLiteFile($config['database.database']);
                ...
                case 'pdo.pgsql':
                    ...
                    if ($config['database.charset']) {
                        $dsn .= ';client_encoding=' . $config['database.charset'];
    ...
    public function getInfo(): array
    {
        ...
        switch ($this->driver) {
            case 'sqlite':
                $info['file'] = $this->connection->query("PRAGMA database_list")->fetchColumn(2);
                break;
            ...
            case 'mysql':
                ...
                if ($collation) {
                    $info['charset']   = explode('_', $collation)[0];
     ...
     * Get
     */
    protected function getSQLiteFile(string $name): string
    {
        $dir  = SYSTEM_STORAGE . config('database-sqlite.path');
        $file = $dir . $name . '.sq3';
    ...
    public function resolveSchema(): string
    {
        return match ($this->driver) {
            'sqlite' => \Junco\Database\Adapter\Sqlite\Schema\Schema::class,
            'pgsql'  => \Junco\Database\Adapter\Pgsql\Schema\Schema::class,
            default  => \Junco\Database\Adapter\Mysql\Schema\Schema::class,
        };
    }
}
...
class AdapterStatement implements StatementInterface
{
    ...
    {
        return new AdapterResult($this->stmt);
    }
